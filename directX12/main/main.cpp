...
void Init(HWND hw) {
    ...
    D3D12_ROOT_SIGNATURE_DESC rd = { 0,0,0,0,D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT };
    ...
    const char* c = "struct V{float4 p:SV_POSITION;float4 c:COLOR;};V VS(float4 p:POSITION,float4 c:COLOR){V o;o.p=p;o.c=c;return o;}float4 PS(V i):SV_TARGET{return i.c;}";
...
cl->SetGraphicsRootSignature(rs); cl->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
WNDCLASSW wc = { 0, WndProc, 0, 0, hI, 0, 0, 0, 0, L"C" }; RegisterClass(&wc);
